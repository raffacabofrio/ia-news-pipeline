...
- [x] Task 6: Verify against the live local stack and close the README forward-reference (AC: 7, 8)
  ...
  - [x] Poll "Get Job Status" (manually or via Postman Runner/Collection Runner) until the job reaches `published` or `failed` at least once — **achieved during code review** (2026-07-07), after the separate worker Dockerfile fix (`11a208d`) landed; see Review Findings below
...
- [x] [Review][Note] Ran a Blind Hunter / Edge Case Hunter / Acceptance Auditor pass over `postman/ia-news-pipeline.postman_collection.json` and `postman/ia-news-pipeline.postman_environment.json` against `service/IaNewsPipeline.Api/Security/HmacRequestValidator.cs`, `Program.cs`, `Contracts/GeneratePostContracts.cs`, `Jobs/JobStates.cs`, and `Validation/PublicUrlValidator.cs`: signed-message format (`timestamp + "." + resolvedBody`), header names/casing, `sha256=` + lowercase-hex encoding, `pm.variables.replaceIn()` used before signing, GET-request signing skip, response-shape assertions (`job_id`/`status_url` on `202`, `job_id`/`state` on `200`, no body assertion on `401`), and the documented `state` enum all match the frozen implementation exactly. No functional defects found; no patch was necessary.
- [x] [Review][Note] Independently reproduced all 3 known vectors from `service/IaNewsPipeline.Tests/HmacSigningTests.cs` two ways rather than trusting the Dev Agent Record's claim: (1) Node's built-in `crypto.createHmac('sha256', secret).update(timestamp + '.' + body).digest('hex')`; (2) the actual `crypto-js` npm package running the literal `CryptoJS.HmacSHA256(timestamp + '.' + rawBody, secret).toString()` expression used in the shipped pre-request script. Both matched all 3 vectors byte-for-byte (64-char lowercase hex each), confirming the collection's signing logic is correct, not just plausible-looking.
- [x] [Review][Note] Independently re-ran the story's `newman` verification claim from a clean baseline: `dotnet test service/IaNewsPipeline.sln` (45/45 green) before touching Docker, then `docker compose up -d --build` (full stack including `worker`, now buildable/startable after `11a208d`'s Dockerfile fix), then `npx newman run postman/ia-news-pipeline.postman_collection.json -e postman/ia-news-pipeline.postman_environment.json --env-var "PIPELINE_SHARED_SECRET=replace-me"`. Result matched the Dev Agent Record exactly: 3/3 requests, 5/5 assertions — `Generate Post — Happy Path` → `202` with `job_id`/`status_url` captured, `Generate Post — Bad Signature` → `401` (its request-level script's hardcoded `wrong-secret` reliably overwrites the collection-level correct signature per Postman's collection-then-request script execution order), `Get Job Status` → `200` with `job_id`/`state`.
- [x] [Review][Note] **Closed AC7 for real.** With `11a208d` already in the tree, the `worker` container started successfully (previously it exited immediately — `docker ps -a` now shows `Up`, not `Exited (150)`). Worker logs showed genuine processing of the happy-path job: `Received queued job` → `Marked job as processing` → HTTP `GET https://example.com/article` → `404` → `Marked job as failed: source_not_found`. Re-ran the "Get Job Status" request itself via `newman --folder "Get Job Status"` against that same `job_id` and observed the terminal state through the actual Postman request path: `state: "failed"`, not just a raw API call. This closes the one gap the Dev Agent Record had honestly left open (Task 6's second subtask, now checked above). No code change was needed — the fix that unblocked this was already delivered and reviewed separately in `11a208d`.
- [x] [Review][Note] Noted (and dismissed as noise, not a functional issue) a newman console deprecation notice on every signed request: `Using "CryptoJS" is deprecated. Use "require('crypto-js')" instead.` This is a newman-CLI sandbox notice about the legacy global-scope `CryptoJS` access pattern, not a Postman Collection Format validation warning — the collection parsed and executed correctly, and the story's own Dev Notes already made a deliberate, documented choice to use the global (`"CryptoJS is available globally in the Postman sandbox (no require/pm.require needed)"`), which remains functionally correct today. No action taken; flagging only so a future maintainer isn't surprised if newman drops the global entirely in some future major version.
...
- 2026-07-07: Code review (unattended, autonomous). Blind Hunter / Edge Case Hunter / Acceptance Auditor pass found no functional defects — HMAC scheme, header names, response-shape assertions, and state-enum documentation all verified against the frozen implementation. Independently reproduced all 3 `HmacSigningTests.cs` vectors via both Node's built-in `crypto` and the real `crypto-js` library (byte-for-byte match). Independently re-ran the full `newman` verification from a clean `dotnet test` baseline (45/45 green) and confirmed 5/5 assertions across all 3 flows. Closed AC7 for real: with the separately-reviewed worker Dockerfile fix (`11a208d`) now in the tree, brought up the full stack, observed the worker genuinely process the happy-path job to a terminal `failed` state (`source_not_found`), and confirmed this through the actual "Get Job Status" Postman request via `newman`, not just a raw API call. No patches, no deferrals — this story is a clean pass. Status moved to `done`.
